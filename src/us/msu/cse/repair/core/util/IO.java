...
public class IO {
	...
	public static void printModificationPoint(ExtendedModificationPoint mp, String filePath) throws IOException {
		String infoStr = mp.getSourceFilePath() + "\t" + mp.getLCNode().getLineNumber() + "\t" + mp.getSuspValue()
				+ "\n";
		infoStr += mp.getStatement().toString();
		infoStr += "********************************************\n\n";

		infoStr += mp.getReplaceIngredients().size() + "\n";	
		for (Statement statement : mp.getReplaceIngredients()) {
			infoStr += statement.toString();
			infoStr += "********************************************\n";
		}
		
		infoStr += mp.getInsertIngredients().size() + "\n";	
		for (Statement statement : mp.getInsertIngredients()) {
			infoStr += statement.toString();
			infoStr += "********************************************\n";
		}
	...
	public static void savePatch(List<Integer> opList, List<Integer> locList, List<Integer> ingredList,
			List<ExtendedModificationPoint> modificationPoints, List<List<String>> availableManipulations, String patchDir,
			int globalID, int evaluations, long estimatedTime, boolean flag) throws IOException {
		File file = new File(patchDir, "Patch_" + globalID + ".txt");
		if (file.exists())
			file.delete();
		...
		for (int i = 0; i < locList.size(); i++) {
			...
			ExtendedModificationPoint mp = modificationPoints.get(loc);
			int lineNumber = mp.getLCNode().getLineNumber();
			String sourceFilePath = mp.getSourceFilePath();
			...
			Statement seed = null;
			if (manipulationName.equals("Replace")
					&& mp.getReplaceIngredients() != null && !mp.getReplaceIngredients().isEmpty())
				seed = mp.getReplaceIngredients().get(ingred);
			else if (manipulationName.equals("InsertBefore")
					&& mp.getInsertIngredients() != null && !mp.getInsertIngredients().isEmpty())
				seed = mp.getInsertIngredients().get(ingred);
